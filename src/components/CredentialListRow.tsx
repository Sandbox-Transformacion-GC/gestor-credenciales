...
export default function CredentialListRow({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className={`grid ${LIST_GRID_COLS} items-center gap-x-3 gap-y-1 rounded-lg border border-slate-200 bg-white px-3 py-2 shadow-sm transition hover:shadow-md dark:border-slate-700 dark:bg-slate-800 ${draggable ? 'cursor-grab active:cursor-grabbing' : ''}`}
    >
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="truncate text-sm font-semibold text-slate-900 dark:text-slate-100" title={credential.title}>
            {credential.title}
          </span>
          {isRestricted && <span title="Solo personas específicas pueden ver esta contraseña" className="shrink-0 text-xs">🔒</span>}
        </div>
        <span className={`inline-block rounded-full px-1.5 py-0.5 text-[10px] font-medium ${categoryBadgeClasses(categoryColor(categories, credential.category))}`}>
          {credential.category}
        </span>
      </div>
      ...
      <span className="truncate text-xs text-slate-400 dark:text-slate-500">{formatDate(credential.updated_at)}</span>
